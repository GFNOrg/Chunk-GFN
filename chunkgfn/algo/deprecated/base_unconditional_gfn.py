...
class UnConditionalSequenceGFN(ABC, LightningModule):
    ...
    def go_backward(self, final_state: torch.Tensor):
        ...
        if self.hparams.pb == "kolya":
            ...
            logNs, mask_back_actions = self.trainer.datamodule.compute_logNs(
        ...
        while not done.all():
            ...
            if self.hparams.pb == "kolya":
                ...
                logp_b_s = self.trainer.datamodule.get_logpb_states(
                    strings,
                    terminal_strings,
                    self.hparams.alpha_pb,
                    logNs,
                    mask_back_actions,
                )
                ...
                action_len = (
                    self.trainer.datamodule.action_len.unsqueeze(0)
                    .repeat(bs, 1)
                    .to(state)
                )
    ...
    def get_ll(
        ...
        if self.hparams.pb == "kolya":
            ...
            str_trajectories = self.trainer.datamodule.to_strings_traj(
                trajectories, dones
            )
            terminal_strings = [
                str_trajectories[i * self.hparams.n_trajectories][-1] for i in range(bs)
            ]
            # Compute logN
            logNs, _ = self.trainer.datamodule.compute_logNs(
                terminal_strings, alpha=self.hparams.alpha_pb
            )
            # Repeat for each trajectory
            logNs = [
                logNs[i]
                for i in range(len(unique_final_state))
                for _ in range(self.hparams.n_trajectories)
            ]
            log_pb = self.trainer.datamodule.get_logpb_trajs(
                str_trajectories, self.hparams.alpha_pb, logNs
            )
