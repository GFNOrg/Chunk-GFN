...
class TBGFN_Variable(UnConditionalSequenceGFN):
    ...
    def update_library(self):
        ...
        new_chunk = self.trainer.datamodule.chunk(samples["actions"], samples["dones"])
        # Sample backward trajectories for the samples before adding the new chunk
        backward_trajectories = self.go_backward(
            self.trainer.datamodule.preprocess_states(samples["final_state"])
        )
        ...
        self.backward_model.logits_layer = expand_linear_layer(
            self.backward_model.logits_layer,
            new_out_dim=self.backward_model.logits_layer.out_features + 1,
            init_weights=init_weights,
        )
        # Reinitialize the optimizer
        self.trainer.optimizers = [self.configure_optimizers()["optimizer"]]
        
        # Sample backward trajectories for the samples after adding the new chunk
        add_logits = torch.zeros(self.backward_model.logits_layer.shape[1]) #TODO : verifier que c est bien ca 
        add_logits[-1] += 5 # increase the logits 
        new_backward_trajectories = self.go_backward(
            self.trainer.datamodule.preprocess_states(samples["final_state"])
